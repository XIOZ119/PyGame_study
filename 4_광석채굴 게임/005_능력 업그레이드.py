...
광석이미지리스트 = []
for 인덱스 in range(5):
    광석이미지 = pg.image.load(f'4_광석채굴 게임/img/광석_{5 - 인덱스}.png')
...
# 오른쪽 하단 능력치 아이콘 이미지
능력치이미지크기 = (545, 190)
능력치이미지 = pg.image.load(f'4_광석채굴 게임/img/능력치.png')
...
while 실행여부:
    ...
    화면.blit(시간이미지, (30, 10))
    화면.blit(게임시작시간글자, (시간이미지크기[0] - 21 * len(시간문자열), 35))

    코인문자열 = str(코인)
    코인글자 = 글꼴.render(코인문자열, True, 검은색)
    화면.blit(코인이미지, (화면가로길이 - 30 - 코인이미지크기[0], 10))
    화면.blit(코인글자, (화면가로길이 - 70 - 21 * len(코인문자열), 35))
    ...
    화면.blit(파워글자, (450, 화면세로길이 - 능력치이미지크기[1] + 55))
    속도글자 = 작은글꼴.render(str(소울곰속도업그레이드횟수), True, 검은색)
    화면.blit(속도글자, (560, 화면세로길이 - 능력치이미지크기[1] + 55))
    광석글자 = 작은글꼴.render(str(광석자동생성업그레이드횟수), True, 검은색)
    화면.blit(광석글자, (670, 화면세로길이 - 능력치이미지크기[1] + 55))

    # 광석자동생성
    광석자동생성남은시간 -= 흐른시간
    if 광석자동생성남은시간 <= 0:
        생성위치 = [random.random() * (화면가로길이 - 게임요소크기[0]), random.random()
                * (화면세로길이 - 시간이미지크기[1] - 능력치이미지크기[1] - 게임요소크기[1]) + 시간이미지크기[1]]
    ...
    for 이벤트 in pg.event.get():
        if 이벤트.type == pg.QUIT:
            ...
            if 이벤트.key == pg.K_z:
                ...
                if 소울곰속도업그레이드횟수 <= 소울곰속도업그레이드최대횟수 and 코인 >= 소울곰파워업그레이드횟수 ** 2:
                    코인 -= 소울곰속도업그레이드횟수 ** 2
                    소울곰속도업그레이드횟수 += 1
                    소울곰속도 += 40
            elif 이벤트.key == pg.K_c:
                if 광석자동생성업그레이드횟수 <= 광석자동생성업그레이드최대횟수 and 코인 >= 소울곰파워업그레이드횟수 ** 2:
                    코인 -= 광석자동생성업그레이드횟수 ** 2
                    광석자동생성업그레이드횟수 += 1
                    광석자동생성시간 += 1
                        ...
                        for 광석스프라이트 in 광석스프라이트리스트:
                            if pg.sprite.collide_mask(소울곰스프라이트, 광석스프라이트):
                                ...
                                if 광석스프라이트.상태 <= 0:
                                    ...
                                    광석스프라이트.image = 광석이미지리스트[math.ceil(
                                        광석스프라이트.상태) - 1]
                                break
        else:
            이미지움직임시간 = 이미지움직임최대시간
