...
요리이미지크기딕셔너리 = {'사료': (100, 55), '뼈다귀': (100, 55), '물': (100, 55)}
요리이미지딕셔너리 = {}
...
동족원하는음식을받음이미지 = pg.image.load('3_부족들의 마음을 요리로 잡아라 게임/img/동족_원하는-음식을-받음.png')
동족원하는음식을받음이미지 = pg.transform.scale(동족원하는음식을받음이미지, 동족이미지크기)

동족원하지않는음식을받음이미지 = pg.image.load(
    '3_부족들의 마음을 요리로 잡아라 게임/img/동족_원하지-않는-음식을-받음(화남).png')
동족원하지않는음식을받음이미지 = pg.transform.scale(동족원하지않는음식을받음이미지, 동족이미지크기)

동족이미지리스트 = [동족음식생각중이미지, 동족원하는음식을받음이미지, 동족원하지않는음식을받음이미지]

동족스프라이트리스트 = [스프라이트생성(동족이미지리스트[0], (85+동족이미지크기[0]*j, 320+동족이미지크기[1]*i), 0, random.random()*(손님생각최대시간-1))
    ...
    if 현재챕터 <= 최종챕터:
        ...
        for 동족스프라이트 in 동족스프라이트리스트:
            ...
            if 동족스프라이트.상태 == 1:
                ...
                음식위치 = [동족스프라이트.rect.x + 95 -
                        (음식크기[0] // 2), 동족스프라이트.rect.y + 50 - (음식크기[1] // 2)]
