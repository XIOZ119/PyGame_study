...
def 당근초기화():
    global 랜덤당근, 랜덤당근좌표
    랜덤당근 = [당근] * 4 + [상한당근] * 5
    랜덤당근좌표 = [(150 + x*230, 120 + y*200) for x in range(3) for y in range(3)]
    random.shuffle(랜덤당근)


pg.init()

화면가로길이, 화면세로길이 = (900, 800)
...
# 이미지 초기화
배경이미지 = pg.image.load('2_상한 당근 게임/img/당근_배경.png')
배경이미지 = pg.transform.scale(배경이미지, (화면가로길이, 화면세로길이))
화면.blit(배경이미지, (0, 0))

# 시간바
시간바 = pg.image.load('2_상한 당근 게임/img/시간바.png')
시간바 = pg.transform.scale(시간바, (400, 100))
화면.blit(시간바, (520, 20))

# 당근
당근 = pg.image.load('2_상한 당근 게임/img/당근.png')
당근 = pg.transform.scale(당근, (130, 220))
#화면.blit(당근, (200, 200))

# 상한당근
상한당근 = pg.image.load('2_상한 당근 게임/img/상한당근.png')
...
while True:
    if 현재챕터 <= 최종챕터:
        ...
        당근글자 = 글꼴.render(
            f'남은 상한 당근 갯수 : {len(랜덤당근) - 랜덤당근.count(당근)}', True, (255, 255, 255))
        화면.blit(당근글자, (80, 90))
        ...
        화면.fill((255, 255, 255))
        종료멘트 = 글꼴.render(f'경과시간은 {경과시간//60}분 {경과시간%60, 2}초 입니다.', True, (0, 0, 0))
