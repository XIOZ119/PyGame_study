...
def 게임요소초기화():
    ...
    global 요소들속도, 돌맹이생성시간, 돌맹이시작높이, 돌맹이위치리스트, 돌맹이스프라이트리스트
...
개리동료이미지 = pg.image.load('5_동족을 노역장에서 구출하라 게임/img/동료-구출-전.png')
개리동료이미지 = pg.transform.scale(개리동료이미지, (100, 100))
개리동료구출이미지 = pg.image.load('5_동족을 노역장에서 구출하라 게임/img/동료-구출-후.png')
개리동료구출이미지 = pg.transform.scale(개리동료구출이미지, (100, 100))
개리동료스프라이트 = 스프라이트_생성(개리동료이미지)

포탈이미지 = pg.image.load('5_동족을 노역장에서 구출하라 게임/img/포탈.png')
포탈이미지 = pg.transform.scale(포탈이미지, (100, 100))
포탈스프라이트 = 스프라이트_생성(포탈이미지)

재시작버튼이미지 = pg.image.load('5_동족을 노역장에서 구출하라 게임/img/다시하기-버튼.png')
...
while 실행여부:
    if 게임여부:
        ...
        재시작버튼 = 화면.blit(재시작버튼이미지, (600, 300))
        ...
        for 돌맹이위치, 돌맹이스프라이트 in zip(돌맹이위치리스트, 돌맹이스프라이트리스트):
            ...
            if (개리동료위치[0] > 15 and pg.sprite.collide_mask(돌맹이스프라이트, 개리동료스프라이트)) or pg.sprite.collide_mask(돌맹이스프라이트, 포탈스프라이트):
                돌맹이위치[0] -= random.random() * 100 + 100
